...
if(isset($_POST['delete'])){
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `product` WHERE id = ?");
   $verify_delete->execute([$delete_id]);

   if($verify_delete->rowCount() > 0){
      $select_images = $conn->prepare("SELECT * FROM `product` WHERE id = ?");
      ...
      $delete_listings = $conn->prepare("DELETE FROM `product` WHERE id = ?");
...
<section class="listings">
   ...
   <?php
      if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
         ...
         $select_listings = $conn->prepare("SELECT * FROM `product` WHERE product_name LIKE '%{$search_box}%' OR address LIKE '%{$search_box}%' ORDER BY date DESC");
         $select_listings->execute();
      }else{
         $select_listings = $conn->prepare("SELECT * FROM `product` ORDER BY date DESC");
         $select_listings->execute();
      } 
      $total_images = 0;
   ...
   <div class="box">
      ...
      <p class="price"><i class="fas fa-rupiah-sign"></i><?= $fetch_listing['price']; ?></p>
      <h3 class="name"><?= $fetch_listing['product_name']; ?></h3>
      ...
      <form action="" method="POST">
         ...
         <a href="view_product.php?get_id=<?= $listing_id; ?>" class="btn">view product</a>
   ...
   <?php
         ...
         echo '<p class="empty">no product posted yet!</p>';
      }
   ?>
