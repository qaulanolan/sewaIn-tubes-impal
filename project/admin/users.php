...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      $select_images = $conn->prepare("SELECT * FROM `product` WHERE user_id = ?");
      ...
      $delete_listings = $conn->prepare("DELETE FROM `product` WHERE user_id = ?");
   ...
   <?php
      ...
      if($select_users->rowCount() > 0){
         while($fetch_users = $select_users->fetch(PDO::FETCH_ASSOC)){
            ...
            $count_product = $conn->prepare("SELECT * FROM `product` WHERE user_id = ?");
            $count_product->execute([$fetch_users['id']]);
            $total_properties = $count_product->rowCount();
   ?>
   <div class="box">
